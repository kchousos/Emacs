...
Adds the option to specify explicitly the root of a project by using a hidden file. This way, the built-in =project.el= will always work and won't rely only on heuristics.

#+begin_src emacs-lisp :tangle ./init.el
(use-package project
  :straight (:type built-in)
  :config
  (defvar project-root-markers '(".project")
